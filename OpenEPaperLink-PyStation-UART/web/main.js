...
function updateTags() {
  ...
  Object.keys(alltagsdata).forEach(function (key) {
    ...
    const timediff =
      Math.floor(Date.now() / 1000) - parseInt(data.lastCheckin, 10);
    // console.log(
    //   Date.now() +
    //     " | " +
    ...
    $("#" + id + " .batteryMv").html("Battery: " + data.batteryMv / 100 + "V");
    $("#" + id + " .lastPacketLQI").html("LQI: " + data.lastPacketLQI + "%");
    $("#" + id + " .lastPacketRSSI").html(
      "RSSI: " + data.lastPacketRSSI + "dBm"
    );
